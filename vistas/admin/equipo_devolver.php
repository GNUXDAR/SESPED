        ...
        <div id="pad-wrapper" class="form-page">
            ...
            <div class="row form-wrapper">
                ...
                <div id="miPagina" class="col-md-8 column"><!--segunda columna de centrado-->
					<h2 align="center">Devolver Equipo</h2></br></br>

                    <form method="POST" action="../admin/equipo_devolver.php">
...
$verificaPersona = pg_query($conectando->conectar(), $buscar) or die('ERROR AL BUSCAR DATOS: ' . pg_last_error());
$localizarPersona=pg_num_rows($verificaPersona);
if ($localizarPersona == 0){
    print ("<script>alert('Persona No Inscrito');</script>");
}
else{
$persona=pg_fetch_array($verificaPersona);
$idPersona = $persona['id_persona'];

$buscar="SELECT * FROM personas  join prestamo on personas.id_persona =prestamo.id_persona join equipos
on equipos.id_equipos = prestamo.id_equipos WHERE prestamo.status=4 and prestamo.id_persona= $idPersona ";
...
if($localizar>0){
    ...
    echo '<div class="row">
    <div class="col-md-7"> <b>Gestionando Devolucion para:</>
     '.$ATRIBUTO['nombre_persona'].'
     '.$ATRIBUTO['apel_persona'].'
     Maquina: '.$ATRIBUTO['codigo_equipos'].'
      </div>
     </div>';
...
<!-- =============================main container ==============================-->
                    ...
                    <form method="POST" action="../admin/control/equipo_devuelto.php" autocomplete="off">
          <!--  <input name="fecha_prestamo" id="fecha_prestamo" value ="<?//php echo date('Y-m-d');?>" class="form-control" required type="hidden">
                ...
                -->        	    <input name="id_persona" id="id_persona" value ="<?php echo $ATRIBUTO['id_persona'] ?>" class="form-control" required type="hidden">
                                <input name="equipo" id="equipo" value ="<?php echo $ATRIBUTO['id_equipos'] ?>" class="form-control" required type="hidden">
                                <input name="id_prestamo" id="id_prestamo" value ="<?php echo $ATRIBUTO['id_prestamo'] ?>" class="form-control" required type="hidden">

                        
                        <div class="action">
                            <input type="submit"  class="btn-flat" id="registrar" value="Devolver"></input>
...
{
    echo '<div class="row">
    <div class="col-md-7">  '.$persona['nombre_persona'].' '.$persona['apel_persona'].' no tiene prestamos activos</div>
    </div>';
}
}
}
